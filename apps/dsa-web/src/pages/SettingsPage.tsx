...
import { Bot, RadioTower, Star, Zap } from 'lucide-react';
...
import type { SetupStatusResponse, SystemConfigCategory } from '../types/systemConfig';
...
const SettingsPage: React.FC = () => {
  ...
  const setupCards = [
    ...
    ...card,
    check: setupChecks.find((item) => item.key === card.key),
  }));

  return (
    <div className="settings-page min-h-full px-4 pb-6 pt-4 md:px-6">
      <div className="mb-5 overflow-hidden rounded-[1.75rem] border border-cyan/20 bg-[linear-gradient(135deg,hsl(var(--card)/0.98),hsl(var(--card)/0.88))] shadow-[0_24px_70px_hsl(var(--background)/0.52)]">
        <div className="border-b border-white/8 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.18),transparent_28%),radial-gradient(circle_at_top_right,hsl(247_84%_72%/0.16),transparent_30%)] px-5 py-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan/20 bg-cyan/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.24em] text-cyan">
                <Zap className="h-3.5 w-3.5" />
                First-Run Control
              </div>
              <h1 className="text-2xl font-semibold tracking-tight text-foreground md:text-[2rem]">基础配置中心</h1>
              <p className="mt-2 text-sm leading-7 text-secondary-text">
                先补齐 LLM 主渠道、Agent 渠道和自选股。完成后，这套系统就能开始最小可用分析，不需要再手动翻 `.env`。
              </p>
            </div>

            <div className="grid grid-cols-2 gap-3 md:min-w-[320px]">
              <div className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3 backdrop-blur-sm">
                <p className="text-[11px] uppercase tracking-[0.18em] text-muted-text">Startup Readiness</p>
                <p className={`mt-2 text-2xl font-semibold ${setupStatus?.readyForSmoke ? 'text-emerald-300' : 'text-amber-300'}`}>
                  {setupStatus?.readyForSmoke ? 'READY' : 'PENDING'}
                </p>
                <p className="mt-1 text-xs text-secondary-text">是否可开始试跑</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3 backdrop-blur-sm">
                <p className="text-[11px] uppercase tracking-[0.18em] text-muted-text">Missing Required</p>
                <p className="mt-2 text-2xl font-semibold text-foreground">
                  {setupStatus?.requiredMissingKeys.length ?? '--'}
                </p>
                <p className="mt-1 text-xs text-secondary-text">仍待完成项</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 px-5 py-4 md:grid-cols-3">
          {setupCards.map(({ key, title, icon: Icon, category, check }) => {
            const status = check?.status || 'needs_action';
            const accentClass = status === 'configured'
            ...
            return (
              <button
                ...
                className="rounded-2xl border border-white/8 bg-[linear-gradient(180deg,hsl(var(--foreground)/0.05),hsl(var(--foreground)/0.02))] p-4 text-left transition-all duration-200 hover:border-cyan/28 hover:shadow-[0_18px_40px_hsl(var(--primary)/0.08)]"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-muted-text">{category}</p>
                    <h2 className="mt-2 text-base font-semibold text-foreground">{title}</h2>
                  </div>
                  <span className={`inline-flex h-10 w-10 items-center justify-center rounded-xl border ${accentClass}`}>
                    <Icon className="h-4.5 w-4.5" />
                  </span>
                </div>
                <p className="mt-3 min-h-[3rem] text-sm leading-6 text-secondary-text">
                  {check?.message || '等待配置'}
                </p>
                <p className="mt-3 text-xs text-cyan">
                  {status === 'needs_action' ? '立即处理' : '查看详情'}
                </p>
              </button>
            );
          })}
