...
test('waitForBackendExit also resolves when only close fires', async (t) => {
  const mainModule = loadMainModule(t);
  const fakeBackend = new EventEmitter();
  ...
  setTimeout(() => {
    fakeBackend.emit('close', 0, null);
  }, 30);

  await Promise.race([
    mainModule.waitForBackendExit(fakeBackend, 200),
    new Promise((_, reject) => setTimeout(() => reject(new Error('waitForBackendExit did not resolve')), 250)),
  ]);
});
